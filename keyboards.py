"""
УЧАСТНИК 3 — Клавиатуры
Файл: keyboards.py

Что делать:
  - Реализовать все три функции ниже
  - callback_data должны точно совпадать с теми, что обрабатывает Участник 5 в handlers/game.py
    "scam"       → игрок считает, что это скам
    "safe"       → игрок считает, что безопасно
    "play_again" → игрок хочет сыграть ещё раз

  Обработчики этих callback_data находятся в game.py и start.py
"""
