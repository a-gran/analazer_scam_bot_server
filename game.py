"""
УЧАСТНИК 5 — Основная игровая логика
Файл: game.py

Что делать:
  - send_question()   — формирует текст вопроса и отправляет его с клавиатурой
  - handle_answer()   — обрабатывает нажатие «Скам» / «Безопасно»:
      1. Достаём сессию из FSM
      2. Определяем ответ игрока (True / False)
      3. Вызываем process_answer() из answer.py
      4. Если жизни кончились или вопросы закончились → передаём в results
      5. Иначе → показываем следующий вопрос

  Важно:
    - Состояние GameStates.playing — фильтр для handle_answer,
      чтобы кнопки не работали вне игры
    - После обработки ответа обновляем сессию: await state.update_data(session=session)
    - Для перехода к результатам: await state.set_state(GameStates.game_over)
"""
