...
Что делать:
  - Это адаптация оригинального answer.py под бота
  - Основное отличие: вместо print() функции возвращают текст,
    который потом отправляется через await message.answer()
  - process_answer() теперь принимает сессию (dict) и возвращает
    текст результата + обновлённую сессию
  - final_title() осталась почти без изменений
"""
